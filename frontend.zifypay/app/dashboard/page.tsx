...
const BusinessDashboard = () => {
  ...
  const [sortOption, setSortOption] = useState<
    "time-asc" | "time-desc" | "name-asc" | "name-desc"
  >("time-asc");
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const bookingsRes = await fetch(
        `${API_URL}/appointments/${businessId}/total-bookings`
      );
      const bookingsData = await bookingsRes.json();
      setTotalBookings(bookingsData.totalBookings ?? 0);

      const revenueRes = await fetch(
        `${API_URL}/appointments/${businessId}/monthly-revenue`
      );
...
//         `http://localhost:5001/api/v1/business/with-status`,
//         {
//           method: "POST",
...
//       const matchedBusiness = data.businesses.find((b) => b._id === businessId);

//       setIsBussinessActive(matchedBusiness?.isActive ?? false);
//     } catch (err) {
//       setIsBussinessActive(false);
//     }
//   };
  ...
  useEffect(() => {
    const fetchQuickStats = async () => {
      ...
      try {
        ...
        const revenueRes = await fetch(
          `${API_URL}/appointments/${businessId}/today-revenue`
        );
        const revenueData = await revenueRes.json();
        setTodaysRevenue(revenueData.todaysRevenue ?? 0);

        const bookingsRes = await fetch(
          `${API_URL}/appointments/${businessId}/today-bookings`
        );
        const bookingsData = await bookingsRes.json();
  ...
  useEffect(() => {
    const fetchTotalCustomers = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_URL}/appointments/${businessId}/total-customers`
        );
        const data = await res.json();
    ...
    const fetchRatings = async () => {
      ...
      try {
        ...
        const avgRes = await fetch(
          `${API_URL}/appointments/${businessId}/average-rating`
        );
        const avgData = await avgRes.json();
        setAverageRating(avgData.averageRating ?? 0);

        const csRes = await fetch(
          `${API_URL}/appointments/${businessId}/customer-satisfaction`
        );
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        {/* {!isBussinessActive && (
          <p className="bg-red-300 p-4 rounded-md ml-12 mr-12">
            {" "}
            Your bussiness is not activate! Set up your merchant credencials to{" "}
              ...
              <Card className="border-0 shadow-lg bg-gradient-to-br from-yellow-500 to-orange-500 text-white">
                ...
                <CardContent>
                  ...
                  <p className="text-xs opacity-90">
                    Based on {loadingReviews ? "..." : recentReviews.length}{" "}
                    reviews
                  </p>
                </CardContent>
              </Card>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center mt-1 text-sm text-gray-500 truncate">
                              ...
                              <span className="truncate">
                                with {appointment.staff?.name || "Unassigned"}
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-4">
                  ...
                  <Progress
                    value={
                      todaysRevenue && todaysRevenue > 0
                        ? Math.min(100, Math.round((todaysRevenue / 650) * 100))
                        : 0
                    }
                    className="h-2"
                  />
                ...
                <div>
                  ...
                  <Progress
                    value={
                      bookingsToday && bookingsToday > 0
                        ? Math.min(100, Math.round((bookingsToday / 12) * 100))
                        : 0
                    }
                    className="h-2"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {bookingsToday
                      ? `${12 - bookingsToday} slots remaining`
                      : "12 slots remaining"}
                  </p>
                </div>
                <div>
                        ...
                        <div key={review._id} className="p-4 rounded-lg border">
                          <div className="flex items-start space-x-3">
                            <Avatar className="h-10 w-10">
                              <AvatarImage
                                src={review.addedBy?.profilePicture}
                              />
                              <AvatarFallback>
