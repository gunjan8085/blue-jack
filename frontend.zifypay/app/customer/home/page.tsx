...
export default function DashboardPage() {
  ...
  const allTags = Array.from(new Set(businesses.flatMap((b) => b.serviceCategories?.flatMap((cat) => cat.tags) || [])))
  ...
  const fetchBusinesses = async () => {
    try {
      ...
      const prices = result.data.flatMap((b) => b.serviceCategories?.map((cat) => cat.price) || [])
  ...
  useEffect(() => {
    const filtered = businesses.filter((business) => {
      ...
      const matchesSearch =
        ...
        business.serviceCategories?.some((cat) => cat.title.toLowerCase().includes(searchQuery.toLowerCase()))

      // Business type filter
      const matchesBusinessType =
        selectedBusinessTypes.length === 0 || selectedBusinessTypes.includes(business.businessType)

      // City filter
      const matchesCity = selectedCities.length === 0 || selectedCities.includes(business.address.city)
      ...
      const matchesPrice =
        ...
        business.serviceCategories?.some((cat) => cat.price >= priceRange[0] && cat.price <= priceRange[1])

      // Team size filter
      const matchesTeamSize =
        (business.teamSize.min >= teamSizeRange[0] || business.teamSize.max >= teamSizeRange[0]) &&
        (business.teamSize.max <= teamSizeRange[1] || business.teamSize.min <= teamSizeRange[1])

      // Online only filter
      const matchesOnline = onlineOnly === null || business.isOnlineOnly === onlineOnly

      // Tags filter
      const matchesTags =
        selectedTags.length === 0 ||
        (business.serviceCategories?.length > 0 &&
          selectedTags.some((tag) => business.serviceCategories?.some((cat) => cat.tags?.includes(tag))))
        ...
        <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
            ...
            <div className="flex-1">
              {/* Desktop Search and Sort Header */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                  {filteredBusinesses.map((business) => (
                    <Link href={`/business/${business._id}`} key={business._id} className="no-underline">
                      <Card className="group hover:shadow-xl transition-all duration-300 border-0 shadow-sm hover:shadow-md hover:-translate-y-1 flex flex-col h-full">
                        <div className="relative">
                          <img
                          ...
                          {business.isOnlineOnly && (
                            <Badge className="absolute top-2 left-2 bg-green-600 hover:bg-green-700 text-xs">
                              Online Only
                            </Badge>
                          )}
                        ...
                        <CardContent className="p-4 flex flex-col flex-1">
                          ...
                          <div className="flex flex-wrap gap-1 sm:gap-2 mb-3">
                            {business.serviceCategories?.slice(0, 2).map((category) => (
                              <Badge
                                key={category.title}
                                variant="secondary"
                            ...
                            {business.serviceCategories?.length > 2 && (
                              <Badge variant="secondary" className="bg-gray-100 text-gray-600 text-xs">
                                +{business.serviceCategories.length - 2} more
                              </Badge>
                            )}
                          </div>
                          {business.serviceCategories?.[0]?.tags?.length > 0 && (
                            <div className="flex flex-wrap gap-1 mb-3">
                              {business.serviceCategories[0].tags.slice(0, 3).map((tag) => (
                        ...
                        <CardContent className="p-4 sm:p-6">
                          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mb-3 line-clamp-2">{business.about}</p>
                              <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
                                ...
                                <div className="flex items-center">
                                  ...
                                  <span>{business.isOnlineOnly ? "Online Only" : "Physical Location"}</span>
                                </div>
                              </div>
                              {/* Price and Tags */}
                              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                                <div className="text-sm text-gray-600">
                                  {business.serviceCategories?.length > 0 && (
                                    <span>From ${Math.min(...business.serviceCategories.map((c) => c.price))}</span>
                                  )}
                                </div>
